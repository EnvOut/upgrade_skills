...
public class UserFormValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        if (user.getCountry().equalsIgnoreCase("none")) {
            errors.rejectValue("country", "Pattern.userForm.country");
        }
        ...
        if (user.getFramework() == null || user.getFramework().size() < 2) {
            errors.rejectValue("framework", "Pattern.userForm.framework");
        }
        if (user.getSkill() == null || user.getSkill().size() < 3) {
            errors.rejectValue("skill", "Pattern.userForm.skill");
        }
    }
}
